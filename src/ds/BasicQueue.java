...
public class BasicQueue<X> {
	...
	public void enQueue(X item) {
		...
		// otherwise check to see if any items have been added the the queue yet
		else if (size() == 0) {
			front++;
			end++;
			data[end] = item;
	...
	public X deQueue() {
		...
		if (size() == 0) {
			throw new IllegalStateException("Can't dequeue bcoz the queue is empty");
		}
	...
	public X access(int position) {
		if(size() == 0 || position > size()) {
			throw new IllegalArgumentException("No item in the queue or the position is greater than the size of the queue");
		}
		int trueIndex = 0;
		for(int i = front; i< end; i++) {
		...
		throw new IllegalArgumentException("Couldn't get the queue item at position: "+position);
	}
